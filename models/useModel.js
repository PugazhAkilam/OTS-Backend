...
const findUserByEmail = async (email) => {
    ...
    try {
        const result = await pool.request()
            ...
            .query('SELECT * FROM user_Master WHERE email_ID = @email and active_Flag=1 ');
...
async function createOrder(userId, addressId, totalPrice,total_quntity,payment,orderStatus_Id) {
    try {
      const pool = await poolPromise; // Connect to the database using the connection pool
      
      // Create a new SQL request
      const orderRequest = pool.request();
       const deliveryman_id=13
      ...
      const orderResult = await orderRequest
        ...
        .input('orderStatus_Id', sql.Int, orderStatus_Id) // Assuming orderStatus_Id is an integer in the Orders table
        .input('deliveryman_id', sql.Int, deliveryman_id)
        .query(
          `INSERT INTO Orders (user_Code, AddressId, total_price, total_item,order_date,payment,deliveryman_id,orderStatus_Id)
           OUTPUT INSERTED.order_id
           VALUES (@user_id, @AddressId, @total_price,@total_quntity, GETDATE(),@payment,@deliveryman_id,@orderStatus_Id)`
        );
...
const disableUser = async (userCode) => {
  try {
      const pool = await poolPromise;
      const result = await pool.request()
          ...
          `);
      return result.rowsAffected[0]; // Returns the number of rows updated
  } catch (error) {
      throw error;
  }
};
