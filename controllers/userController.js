...
const { generateOrderMessage } = require('../utils');
...
async function placeOrder(req, res) {
  const { cartItems, totalPrice, addressId, userId, totalquantity, payment, userPhoneNumber,orderStatus_Id } = req.body;
 console.log(totalPrice,typeof totalPrice);
 console.log("orderstatusid",orderStatus_Id, typeof orderStatus_Id);
 
  try {
    // Create a new order
    const orderId = await userModel.createOrder(userId, addressId, totalPrice, totalquantity, payment,orderStatus_Id);

    // Create order items associated with the order
    await userModel.createOrderItems(orderId, cartItems);

    // Generate HTML message using the template
    const message = generateOrderMessage(orderId, cartItems, totalPrice);

    // Send WhatsApp message
    await axios.post("https://wav5.algotechnosoft.com/api/send", {
      number: `91${userPhoneNumber}`,
      type: "text",
      message: message,
...
const disableUserByCode = async (req, res) => {
  ...
  try {
      const rowsAffected = await userModel.disableUser(userCode);

      if (rowsAffected > 0) {
          res.status(200).json({ message: `User ${userCode} has been disabled.` });
      } else {
          res.status(404).json({ error: `User with code ${userCode} not found.` });
      }
  } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'An error occurred while disabling the user.' });
  }
};
...
module.exports = { getUsers, addUserAndAddress ,getUserById,deleteUser,
  ...
  getAllOrders,getOrderById,updateOrderStatus,getUserOrders,getOrderDetails,cancelorder,updateAddressController,deleteAddress,disableUserByCode};
